...
def review_list():
    ...
    matched_items    = MatchReview.query.filter(MatchReview.approved.is_(True)).all()
    current_app.logger.info(f"[review_list] No pending reviews. New items to add: {len(new_items_to_add)} | Matched items: {len(matched_items)}")
    return render_template(
        'reviews_done.html',
...
def handle_review(item_id):
    ...
    if request.form.get('decision') == 'approve':
        ...
        flash(f"Approved “{review.new_item.name}”.", "success")
        current_app.logger.info(f"[handle_review] Approved review for item '{review.new_item.name}'")
    else:
...
def batch_save_decisions():
    reviews = MatchReview.query.join(NewItem) \
        .filter(MatchReview.approved.is_(None), NewItem.ignored.is_(False)) \
        .all()
    current_app.logger.info(f"[batch_save_decisions] Saving batch review decisions for {len(reviews)} items")
    for review in reviews:
        decision = request.form.get(f"approve_{review.id}")
        if decision == 'yes':
            review.approved = True
            review.new_item.resolved = True
            if review.suggested_ext_id:
    ...
    db.session.commit()
    flash("Your review decisions have been saved.", "success")
    current_app.logger.info("[batch_save_decisions] Batch review decisions saved.")
    return redirect(url_for('main.review_list'))
...
def push_to_dgraph():
    ...
    def lookup_ref(ref_type_query, var_name, title, id_field):
        ...
        resp = requests.post(url, json={"query": q, "variables": {"title": title}}, headers=headers)
        data = resp.json().get("data", {}).get(ref_type_query, [])
        if data:
            current_app.logger.info(f"[push] Found {id_field} for '{title}': {data[0][id_field]}")
            return {id_field: data[0][id_field]}
        current_app.logger.warning(f"[push] No {id_field} found for '{title}'")
        return None
    ...
    for m in members:
        ...
        try:
            ...
            if not country_ref:
                results["errors"].append(f"Country '{m.country1}' not found for business '{biz}'—skipped.")
                current_app.logger.warning(f"[push] Skipping '{biz}' due to unknown country '{m.country1}'")
                continue
            ...
            if node_list:
                ...
                exist_ps = {p["title"]: p["productID"] for p in node["byProducts"]}
                ...
                if prod_ids or ing_ids:
                    ...
                    v = {
                      "in": {
                        ...
                        "set":    {"byProducts": all_ps, "ingredients": all_is}
                      }
                    }
            ...
            member_input = {
                ...
                "byProducts":     [{"title": t} for t in subs_ps],
